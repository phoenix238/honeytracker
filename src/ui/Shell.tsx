...
type View = 'home' | 'money' | 'docs' | 'more';
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg }}>
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '24px 18px 96px' }}>
        {store.error && (
          <div style={{ background: 'rgba(184,57,47,0.12)', border: `1px solid ${T.danger}`, borderRadius: 12, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        ...
        {view === 'home' && <HomeView store={store} onSeeAll={() => setView('money')} />}
        {view === 'money' && <MoneyView store={store} />}
        {view === 'docs' && <DocsView />}
        {view === 'more' && <MoreView store={store} />}
      </div>
